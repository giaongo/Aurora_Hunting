...
const Weather = ({route}) => {
  ...
  const doFetchWeatherData = async (
    lat,
    long,
    param1,
    param2,
    ...
    try {
      const response = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${long}&current_weather=${true}&temperature_unit=${'celsius'}&${param1}=${param2},weathercode&${param3}=${param4}`
      );
  ...
  const getCoordinates = async () => {
    try {
      ...
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${cityName}&key=${googleApi}`
      );
  ...
  const getHourlyWeather = async (Lat, Long) => {
    try {
      const hourlyWeatherInfo = await doFetchWeatherData(
        Lat,
        Long,
        'hourly',
        'temperature_2m',
  ...
  const getDailyWeather = async (Lat, Long) => {
    try {
      const dailyWeatherInfo = await doFetchWeatherData(
        Lat,
        Long,
        'daily',
  ...
  const mappedHourlyArray = () => {
    const currentTime = Number(
      currentWeather.time?.split('T')[1].split(':')[0]
    );
    return hourlyWeatherTime
      .slice(currentTime, currentTime + 24)
      .map((value, index) => {
        const time = value.split('T')[1].split(':')[0];
        return {
          time: time,
          temp: hourlyWeatherTemp[index] + weatherUnit,
          id: index,
  ...
  const HourlyItem = ({item}) => {
    return (
      <View style={{marginHorizontal: 10, alignItems: 'center'}}>
        ...
        <IconButton icon={weatherCodes[item.code]?.icon} size={30} />
  ...
  const mappedDailyArray = () => {
    return dailyWeatherTime.map((value, index) => {
      const dailyDate = new Date(value)?.toLocaleDateString('en-US', {
        weekday: 'short',
      });
      const today = currentWeather.time?.split('T')[0];
      return {
        time: value === today ? 'Today' : dailyDate,
        minTemp: dailyWeatherTempMin[index],
        maxTemp: dailyWeatherTempMax[index],
        id: index,
    ...
    return (
      <View style={styles.dailyItemContainer}>
        <Text style={{fontSize: 20, width: '25%'}}>
          {' '}
          {item.time === todaysDate ? 'Today' : item.time}{' '}
        </Text>
        <IconButton icon={weatherCodes[item.code]?.icon} size={25} />
  ...
  */
  ...
  const temperature = () => {
    ...
    if (currentWeather.temperature > 12) {
      highColor = Math.round(
        (1 - (currentWeather.temperature - 20) / 28) * 255
      ); // hot weather
      ...
      highColor = Math.round(
        (1 - (currentWeather.temperature + 10) / 32) * 255
      ); // cold weather
      lowColor = highColor - 150;
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <Text style={{fontSize: 20, marginTop: 10}}>
          {' '}
          {new Date(currentWeather.time).toLocaleString('en-US', {
            weekday: 'long',
          }) +
            ', ' +
            new Date(currentWeather.time).getDate('en-US') +
            ' ' +
            new Date(currentWeather.time).toLocaleString('en-US', {
              month: 'long',
            })}
        ...
        <View style={styles.locationContainer}>
          <IconButton icon={'map-marker-radius'} size={35} />
          <Text style={styles.textLocation}>
            {' '}
            {cityNames[0].toUpperCase()}{' '}
          </Text>
        </View>
        ...
        <Text style={{fontSize: 25, marginBottom: 30}}>
          {weatherCodes[currentWeather.weathercode]?.description}{' '}
        </Text>
        <FlatList
